...
desired over *absolute* posteriors. (@eq-bayes-full) then simplifies
to,
...
If we have $p$ features and na&iuml;vely assume all are independent,
we can multiply their individual probabilities to produce a cumulative
...
Naïve Bayes models assume Gaussian densities and are calculated
...
The result of (@eq-bayes) gives a probability *density* for each
class, which is not constrained on the interval $[0,\ 1]$. Normalized
posterior probabilities ($Pr$) are obtained by calculating the
...
example, with $2pk + k = 10$ parameters. The Naïve Bayes model
looks like this:
...
and the feature measurements for Sample 1 ($\vec x =$ `r s1f1`,
`r s1f2`) are represented by the dashed vertical line. This graphically
indicates that Sample 1 is more likely to have come from the control
distribution.
