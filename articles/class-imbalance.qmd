---
title: "Effect of Class Imbalance on Prediction Accuracy"
subtitle: "A cautionary tale of balance"
author: "Stu Field"
format:
  gfm:
    preview-mode: raw
...
- at each iteration the training set is bootstrap sampled from
  class 1 *only*
...
Plot the simulated data based on the
2 predictors (by class)
...
- Simulation starts with class 1 at
  `r round(100 * simres$class_balance[1L], 2L)`%
  and Accuracy = `r round(simres$accuracy[1L], 3L)`.
- Simulation ends with class 1 at
  `r round(100 * simres$class_balance[nrow(simres)], 2L)`%
