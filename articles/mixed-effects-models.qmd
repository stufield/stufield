...
(regressors/variables/proteins). The extension of (@eq-lme) can be
modeled as:
...
the fixed-effect for _time_. However, now there is a fixed-effect for
_group_ and an interaction term ($\beta_3$ term) that is multiplied by
...
_and_ an significant interaction; the additional effect of being in
the _treatment_ group. Relate the values back to the original
(@eq-lme-dummy) above in @sec-group-dependent to see how the terms are
partitioned.
...
interaction coefficient ($\hat{\beta_3}=$ `r b3`) which was carrying
the entire slope effect for the treatment group. Stated alternatively,
the significance of the interaction has dropped, though still a
significant $p-$value, shifting from `r t1` to `r t2`.
