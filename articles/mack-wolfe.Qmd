...
the Mack-Wolfe test counts for _each_ value in Group A, the number of values in
Group B that are greater in magnitude (+0.5 for ties), and repeats this
calculation for all _each_ $n\choose2$, $n\ge2$, pairwise combinations (i.e.
`A--B, A--C, A--D, B--C, B--D, C--D`).  In the extreme, if there are 9 samples
in Group A and 15 samples in Group B, and _all_ the samples in Group B are
greater than the highest value in Group A, this results in $9 \times 15 = 135$
